...
export class Guild {
	...
	constructor(client: Client, data: APIGuild | GatewayGuildCreateDispatchData) {
		...
		if (data.icon)
			this.#icon = BigInt(
				`0x${this.#animated ? data.icon.replace("a_", "") : data.icon}`,
			);
		...
		if (data.widget_channel_id)
			this.widgetChannelId = BigInt(data.widget_channel_id);
		...
		this.applicationCommands = new GuildApplicationCommandManager(
			client,
			this.id,
		);
		if (data.application_id) this.applicationId = BigInt(data.application_id);
		if (data.system_channel_id)
			this.systemChannelId = BigInt(data.system_channel_id);
		this.systemChannelFlags = data.system_channel_flags;
		if (data.rules_channel_id)
			this.rulesChannelId = BigInt(data.rules_channel_id);
		// @ts-expect-error
		if (data.joined_at)
			// @ts-expect-error
		...
		if (data.public_updates_channel_id)
			this.publicUpdatesChannelId = BigInt(data.public_updates_channel_id);
		...
		const member = client.cache.members.get(this.id, client.user.id);
		if (member) this.me = member;
	}
	...
	async createChannel(data: ChannelOptions, reason?: string) {
		const request = (await this.client.rest.post(
