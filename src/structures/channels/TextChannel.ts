...
export class TextChannel extends GuildChannel {
	...
	async send(options: RawMessageOptions) {
		const parsedData = new MessageData({
			allowedMentions: this.client.allowedMentions,
			...options,
		});
		const request = this.client.rest.post(
	...
	async bulkDelete(amount: number) {
		if (amount < 2 || amount > 100)
			throw new APIError(
				"You can only bulk delete between 2 and 100 messages.",
			);
	...
	async getPublicArchivedThreads() {
		const request = (await this.client.rest.get(
			`/channels/${this.id}/threads/archived/public`,
		)) as any;
		if (request?.code) {
			throw new APIError(request?.message);
		}
	...
	async getPrivateArchivedThreads() {
		const request = (await this.client.rest.get(
			`/channels/${this.id}/threads/archived/private`,
		)) as any;
		if (request?.code) {
			throw new APIError(request?.message);
		}
	...
	async getJoinedPrivateArchivedThreads() {
		const request = (await this.client.rest.get(
			`/channels/${this.id}/users/@me/threads/archived/private`,
		)) as any;
		if (request?.code) {
			throw new APIError(request?.message);
		}
