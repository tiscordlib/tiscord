...
export class RESTClient {
	...
	public static getDefaultOptions(token: string): Required<RESTClientOptions> {
		return {
			...
			version: 10,
			auth: `Bot ${token}`,
			userAgent: "DiscordBot (https://github.com/tiscordlib/tiscord; 0.0.0)",
			headers: {},
		};
	}
	...
	public createHeaders(request: APIRequest): Record<string, string> {
		...
		if (request.reason?.length) headers["x-audit-log-reason"] = request.reason;
		if (request.locale?.length) headers["x-discord-locale"] = request.locale;
