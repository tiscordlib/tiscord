...
export function resolveColor(color) {
    if (typeof color === 'string') {
        ...
        if (/^#?[\da-f]{6}$/u.test(color)) return parseInt(color.replace('#', ''), 16);
        color = Colors[color];
    } else if (Array.isArray(color)) {
        color = (color[0] << 16) + (color[1] << 8) + color[2];
    }

    if (color < 0 || color > 0xffffff) throw new Error('Color must be within the range 0 - 16777215 (0xFFFFFF).')
    if (typeof color !== 'number' || Number.isNaN(color)) throw new Error('Unable to convert color to a number.');
