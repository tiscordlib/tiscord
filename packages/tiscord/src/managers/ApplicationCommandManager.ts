import { APIApplicationCommand, RESTPatchAPIApplicationCommandJSONBody } from 'discord-api-types/v10';
...
export class ApplicationCommandManager {
    ...
    async get(id: string) {
        return new ApplicationCommand(
            await this.client.rest.get(`/applications/${this.client.applicationId}/commands/${id}`)
        );
    }
    ...
    async edit(id: string, command: RESTPatchAPIApplicationCommandJSONBody) {
        return new ApplicationCommand(
            await this.client.rest.patch(`/applications/${this.client.applicationId}/commands/${id}`, {
                body: command
            })
        );
    }
    ...
    async delete(id: string) {
        return this.client.rest.delete(`/applications/${this.client.applicationId}/commands/${id}`);
    }
    ...
    async bulkEdit(commands: APIApplicationCommand[]) {
        return (
            (await this.client.rest.put(`/applications/${this.client.applicationId}/commands`, {
                body: commands
            })) as APIApplicationCommand[]
        ).map((e: any) => new ApplicationCommand(e));
    }
    ...
    async create(command: APIApplicationCommand) {
        return new ApplicationCommand(
            await this.client.rest.post(`/applications/${this.client.applicationId}/commands`, { body: command })
        );
    }
}
