...
import { DefaultDiscordOptions } from './index';
import { RequestManager, RequestManagerOptions, RouteLike } from './RequestManager';
...
export class REST extends RequestManager {
    ...
    async request<T>(options: APIRequest & { buf?: boolean }): Promise<T> {
        ...
        if (task && typeof task === 'object' && !Array.isArray(task) && task !== null) {
            Object.assign(options, task);
        }

        const res = await this.queue(options);

        if (options.buf) return res.body.arrayBuffer().then(Buffer.from) as Promise<T>;
