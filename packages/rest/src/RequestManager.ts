...
import { APIRequest, resolveRequest } from "./APIRequest';
...
export class RequestManager {
  ...
  public getBucket(route: RouteLike) {
    const majorId =
      /(?:channels|guilds|webhooks)\/(\d{17,19})/u.exec(route)?.[1] ?? "global";
        ...
        case 429: {
          ...
          if (res.headers["x-ratelimit-global"]) {
            ...
            this.remaining = Number(
              res.headers["x-ratelimit-global-remaining"]
            );
  ...
  private updateHeaders(res: ResponseData) {
    ...
    if (
      Boolean(res.headers["x-ratelimit-bucket"]) ||
